 ...
 * m = [1,2,4,5,6] n = [7,9,10]
 * m = [1,2,4,5] n = [7,9,10]
 * 我们分别找第 (m+n+1) / 2 个，和 (m+n+2) / 2 个，然后求其平均值即可，这对奇偶数均适用。
 * 假如 m+n 为奇数的话，那么其实 (m+n+1) / 2 和 (m+n+2) / 2 的值相等，
 ...
 * 还有就是如果K=1的话，
 * 那么我们只要比较nums1和nums2的起始位置i和j上的数字就可以了。
 * 难点就在于一般的情况怎么处理？
 * 因为我们需要在两个有序数组中找到第K个元素，为了加快搜索的速度，
 * 我们要使用二分法，对K二分，
...
public class Test {
    ...
    /**
     * 功能描述: i = nums1 的起始位置， j = nums2的起始位置
     * @Author ZYC
     * @Date 2021/5/21 10:24
     * @Param [nums1, i, nums2, j, k]
     * @Return int
     * @Version 1.0
     **/
    ...
    public static void main(String[] args) {
        ...
        int m = num2.length;
        int[] arr = new int[m+n];
        System.arraycopy(num1, 0, arr, 0, n);
        System.arraycopy(num2, 0, arr, n, m);
        Arrays.sort(arr);
        int a = (m+n)/2;
        if(((m+n)%2)!=0){
            ...
            System.out.println((arr[(m+n)/2]+arr[a-1])*1.0/2);
        }
    }
}
